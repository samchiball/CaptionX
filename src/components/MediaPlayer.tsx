...
interface Props {
  /** Absolute audio path (for waveform extraction) */
  audioPath: string | null
  /** Playable audio URL (captionx-media://...), null if not ready */
  src: string | null
  /** Message for preparing/failed state */
...
/**
 * Audio player for verifying subtitle timing.
 * Since the original video codec may not be decodable in the browser,
 * it plays the audio (m4a) extracted by the backend. ref is used in useMediaSync to track currentTime/seek.
 */
export const MediaPlayer = forwardRef<HTMLMediaElement, Props>(function MediaPlayer(
  ...
  useEffect(() => {
    ...
    const handleWaiting = (): void => {
      console.warn('[CaptionX] audio waiting', {
    ...
    const handleStalled = (): void => {
      console.warn('[CaptionX] audio stalled', {
        readyState: audioEl.readyState,
        networkState: audioEl.networkState,
        currentTime: audioEl.currentTime
      })
    ...
    const handleEmptied = (): void => {
      console.warn('[CaptionX] audio emptied — src cleared')
    }
  ...
  return (
    <div className="media-player-container">
      ...
      <audio ref={setAudioRef} src={src || undefined} preload="auto" style={{ display: 'none' }} />
