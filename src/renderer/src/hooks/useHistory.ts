...
export function useHistory(): HistoryApi {
  ...
  const refresh = useCallback(async (): Promise<void> => {
    ...
    setLoading(true)
    try {
      setEntries(await window.api.historyList())
    } finally {
      setLoading(false)
    }
  }, [])
  ...
  const load = useCallback((id: string) => window.api.historyLoad(id), [])

  // 적재된 항목은 라이브 작업과 동일하게 resplit/exportSubtitle IPC 로 처리된다.
  const resplit = useCallback(
    (id: string, options: ResplitOptions) => window.api.resplit(id, options),
    []
  )

  const exportEntry = useCallback(
    (id: string, options: ExportOptions) => window.api.exportSubtitle(id, options),
    []
  )
