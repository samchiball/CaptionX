...
export function useUpdater(): UpdaterApi {
  ...
  const check = useCallback(() => {
    window.api.updateCheck().catch((err) => console.error('[useUpdater] check failed:', err))
  }, [])

  const download = useCallback(() => {
    window.api.updateDownload().catch((err) => console.error('[useUpdater] download failed:', err))
  }, [])

  const install = useCallback(() => {
    window.api.updateInstall().catch((err) => console.error('[useUpdater] install failed:', err))
  }, [])
  ...
  return {
    status,
    manualInstall: window.api.platform === 'darwin',
    check,
    download,
    install,
    openReleasePage
