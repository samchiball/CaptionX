...
function App(): React.JSX.Element {
  ...
  // Default 2: Transcribes multiple files in the queue concurrently (warns if recommended limit is exceeded).
  const [concurrency, setConcurrency] = usePersistentState('concurrency', 2)
  // whisper.cpp inference thread count (n_threads). 0=auto (default based on core count).
  ...
  // If the deprecated 'whisper' alignment mode remains in localStorage, migrate to 'wav2vec2'.
  useEffect(() => {
  ...
  // Toggle settings panel with Ctrl+, (mac: Cmd+,), and switch tabs with Ctrl+1/2.
  useEffect(() => {
  ...
  return (
    <div className="app">
      ...
      {tab === 'history' ? (
        ...
        <>
          {/* Keep active so new files can be added to queue even during transcription. */}
